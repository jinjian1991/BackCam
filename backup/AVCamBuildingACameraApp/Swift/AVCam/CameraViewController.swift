...
//		cameraSwitchControl.isHidden = true
		
		cameraSwitchControl.isEnabled = true
		
		previewView.frame = CGRect(x: 125, y: 60, width: 100, height: 40)
		
		photoButton.frame = CGRect(
			x: UIScreen.main.bounds.size.width/2 - 300/2,
			y: UIScreen.main.bounds.size.height-150-270,
			width: 300,
			height: 70)
		
		
		let passFrame = CGRect(x: 20, y: photoButton.frame.minY - 200,
							   width: UIScreen.main.bounds.size.width-20*2, height: 50)
		
		let passField = UITextField(frame: passFrame)
		...
		let nameFrame = CGRect(x: passFrame.minX, 
							   y: passFrame.minY - passFrame.height - 16,
							   width: passFrame.width,
							   height: passFrame.height)
		let nameField = UITextField(frame: nameFrame)
		nameField.layer.cornerRadius = 8
		nameField.placeholder = "请输入用户名"
		nameField.backgroundColor = .systemBlue
		nameField.textColor = .white
		nameField.delegate = self
		view.addSubview(nameField)
		
		view.addSubview(cameraSwitchControl)
		cameraSwitchControl.frame = CGRect(
			x: UIScreen.main.bounds.size.width/2 - 174/2,
			y: photoButton.frame.minY - 16 - 32,
			width: 174,
			height: 32
		)
		cameraSwitchControl.addTarget(self,
									  action: #selector(cameraSwitchAction(cameraControl:)),
									  for: .valueChanged)
	}
	
	@objc func cameraSwitchAction(cameraControl: UISegmentedControl) {
		...
		self.changeCamera(nil, isUserSelection: true, cameraType: cameraControl.selectedSegmentIndex, completion: {
		...
		// 拍照模式
//		if cameraControl.selectedSegmentIndex == CaptureMode.photo.rawValue {
//			recordButton.isEnabled = false
...
//            var defaultVideoDevice: AVCaptureDevice? = AVCaptureDevice.systemPreferredCamera
            ...
            if !userDefaults.bool(forKey: "setInitialUserPreferredCamera") || defaultVideoDevice == nil {
				let backVideoDeviceDiscoverySession = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera, .builtInDualCamera, .builtInWideAngleCamera],
	...
	// 切换背部三颗镜头
	lazy var cameraSwitchControl: UISegmentedControl = {
		let view = UISegmentedControl(frame: .zero)
		view.backgroundColor = .systemBlue
		view.insertSegment(withTitle: "超", at: 0, animated: true)
		view.insertSegment(withTitle: "广", at: 1, animated: true)
		view.insertSegment(withTitle: "长", at: 2, animated: true)
...
//	
//	
//	@IBAction func switchCameraMode(_ sender: UISegmentedControl) {
//		NSLog("switchCameraMode: ", sender)
//	}
	
	/// - Tag: EnableDisableModes
    @IBAction private func toggleCaptureMode(_ captureModeControl: UISegmentedControl) {
        captureModeControl.isEnabled = false
//		cameraSwitchControl.isEnabled = false
//		cameraSwitchControl.isHidden = true
...
//					self.cameraSwitchControl.isHidden = false
//					self.cameraSwitchControl.isEnabled = true
                }
	...
	private func changeCamera(_ videoDevice: AVCaptureDevice?, isUserSelection: Bool, cameraType: Int = 0, completion: (() -> Void)? = nil) {
        sessionQueue.async {
            ...
            if let videoDevice = videoDevice {
				...
				switch cameraType {
					// 超广角
					case 0:
						let ultraTypes = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera, .builtInDualCamera, .builtInWideAngleCamera],
																							   mediaType: .video, position: .back)
						newVideoDevice = ultraTypes.devices.first
						break
					// 广角
					case 1:
						let ultraTypes = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera, .builtInDualCamera, .builtInWideAngleCamera],
																							   mediaType: .video, position: .back)
						newVideoDevice = ultraTypes.devices[1]
						break
					// 长焦
					case 2:
						let ultraTypes = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera, .builtInDualCamera, .builtInTelephotoCamera],
																							   mediaType: .video, position: .back)
						newVideoDevice = ultraTypes.devices[2]
						break
					default:
						break
				}
	...
	private var photoQualityPrioritizationMode: AVCapturePhotoOutput.QualityPrioritization = .quality//.balanced
	
	// 默认超广角
//	private var cameraType = AVCaptureDevice.DeviceType.builtInUltraWideCamera
//	let cameraType: AVCaptureDevice.DeviceType = .builtInUltraWideCamera
//	
//	private var diviceBack = AVCaptureDevice.default(cameraType, for: .video, position: .back)
